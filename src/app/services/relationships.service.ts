...
export class RelationshipsService {
	...
	sortRelationshipsByAttentionNeededDesc(a: Relationship, b: Relationship): number {
	...
	groupRelationshipsByAttentionNeededStatus(relationships: Relationship[]): RelationshipsGroupedByStatus {
	...
	addRelationshipToGroups(newRelationship: Relationship, groups: WritableSignal<RelationshipGroup[]>) {
		groups.update(groups => {
			return groups.map(group => {
				if (group.status === newRelationship.attentionNeededStatus) {
					group.relationships = this.insertRelationshipInOrder(group.relationships, newRelationship)
				}
				return group
			})
		})
	...
	private insertRelationshipInOrder(sortedRelationships: Relationship[], newRelationship: Relationship): Relationship[] {
		let low = 0
	...
	updateRelationshipInGroups(updatedRelationship: Relationship, groups: RelationshipGroup[]): RelationshipsGroupedByStatus {
			...
			relationships.sort(this.sortRelationshipsByAttentionNeededDesc)

		const relationshipsGroupedByStatus = this.groupRelationshipsByAttentionNeededStatus(relationships)
		return relationshipsGroupedByStatus
	}
