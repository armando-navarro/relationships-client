...
import { Interaction, InteractionGroup, TimeUnit } from './interaction-interface'
import { InteractionDialog, InteractionDialogData, InteractionDialogResult } from './interaction-dialog/interaction-dialog'
...
export class Interactions {
	...
	async addInteraction(formService: RelationshipForm): Promise<InteractionDialogResult>
	async addInteraction(arg: Interaction[] | RelationshipForm): Promise<AddInteractionResult | InteractionDialogResult> {
		const customConfig: Partial<InteractionDialogData> = { isAddingInteraction: true }
		const isFormService = arg instanceof RelationshipForm
	...
	async editInteraction(interaction: Interaction, formService: RelationshipForm): Promise<InteractionDialogResult>
	async editInteraction(interaction: Interaction, arg: Interaction[] | RelationshipForm): Promise<InteractionDialogResult | AddInteractionResult> {
		if (arg instanceof RelationshipForm) return this.openInteractionDialog({ interaction, isEditingInteraction: true }, arg)
	...
	private async openInteractionDialog(customDialogConfig: Partial<InteractionDialogData>, formService?: RelationshipForm): Promise<InteractionDialogResult> {
		...
		return lastValueFrom(this.dialog.open(InteractionDialog, config).afterClosed())
	}

	/** @returns `false` if user clicked Cancel, `true` if interaction was deleted */
	deleteInteraction(interaction: Interaction, formService?: RelationshipForm): Observable<boolean> {
